...
function getDefaultMs(entity){
	let entities=entityPages, nextEntities=[];
	let entityParts=entity.split(":");
	for (i=0; entities && nextEntities; i++) {
...
function getCompareFromCollation (indexEntity, nowEntity, ms) {
	let activeMSS=[];
	let folder=indexEntity.slice(0, indexEntity.lastIndexOf(":"));
	let file=indexEntity.slice(indexEntity.lastIndexOf(":")+1);
	let moveJSON={stacked: true, activeMSS: [], place: nowEntity, MS: ms, fromLink: true};
	setCookie("compare", JSON.stringify(moveJSON), 1);  //set for a whole day but keep updating
	window.location.href="../../../html/compare/"+folder+"/"+file+".html";
}

function getCompareFromIndex (indexEntity, nowEntity, ms) {
	let activeMSS=[];
	let folder=indexEntity.slice(0, indexEntity.lastIndexOf(":"));
	let file=indexEntity.slice(indexEntity.lastIndexOf(":")+1);
	let moveJSON={stacked: true, activeMSS: [ms], place: nowEntity, MS: ms, fromLink: true};
	setCookie("compare", JSON.stringify(moveJSON), 1);  //set for a whole day but keep updating
	window.location.href="html/compare/"+folder+"/"+file+".html";
}
