	...
	if (!onlyReg && words) {
		...
		for (var i=0; i<collation.structure.apparatus.length; i++) {
			for (j=0; j<collation.structure.apparatus[i].readings[0].text.length; j++) {
				baseMs="Base";
				if (typeof collation.structure.apparatus[i].readings[0].text[j]["Base"]=="undefined") {
					baseMs=collation.structure.apparatus[i].readings[0].text[j].reading[0];
				}
				if (!transform && collation.structure.apparatus[i].readings[0].text[j][baseMs].pc_before) {
					appText+=collation.structure.apparatus[i].readings[0].text[j][baseMs].pc_before+" ";
				}
				if (!transform) {
					appText+=collation.structure.apparatus[i].readings[0].text[j][baseMs].original;
				}
				if (j<collation.structure.apparatus[i].readings[0].text.length-1 && !transform) appText+=" ";
				if (!transform && collation.structure.apparatus[i].readings[0].text[j][baseMs].pc_after) {
					appText+=collation.structure.apparatus[i].readings[0].text[j][baseMs].pc_after+" ";
				}
			}
			appText+=" ";
		}
