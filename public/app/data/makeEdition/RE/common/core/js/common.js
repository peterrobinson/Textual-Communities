...
//			$("#rTable").height($(window).height()-$("#page-head").height()-12);
	...
	if (view=="compare") {
		$("#panel-right").width("100%");
		$("#rTable").height(""+($(window).height()-$("#page-head").height()-15)+"px");
		if ($("#cfStack").is(":visible")) $("#cfStack").height($("#panel-right").height()-$("#cfLinks").height()-$("#cfHeader").height()-29);
		if ($(".cfImage").is(":visible")) $(".cfImage").height($("#panel-right").height()-$("#cfLinks").height()-$("#cfHeader").height()-29);
		if ($(".cfText").is(":visible")) $(".cfText	").height($("#panel-right").height()-$("#cfLinks").height()-$("#cfHeader").height()-29);
	}
...
//		$("#transcript-tm").width($("#transcript-text").width());   //set width of all transcript elements..
	...
	if ($(".gutter-vertical").length && !$("#searchContainer").is(":visible")) {
		resizeVertically();
...
//			$("#rTable").height(newHeight-$("#footer").height()-3);
	...
	if ($("#OAstatement").is(":visible")) {
		let newHeight=$(window).height()-$("#page-head").height()-24-$("#OAstatement").height();
